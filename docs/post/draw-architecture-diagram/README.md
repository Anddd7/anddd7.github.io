...
语言是人类交流和表达思想的一种工具，它通过语音和符号来传递意义。语言的起源可以追溯到早期人类通过声音和手势进行基本交流的时期。随着人类智慧的发展，语言逐渐演化成为复杂的符号系统，用于表示事物、概念和抽象思维。

文字是语言的可视化形式，用于通过抽象的符号和记号记录和传播信息。文字的发展是为了解决语言的时间和空间限制，使得信息可以被保存、传递和阅读，超越了口头传统的限制。
...
（这里不讨论图形、图像的定义和区别，属于广义的图形）

图形是通过线条、形状、点和颜色等基本要素构成的平面或立体的视觉表达。它们可以是简单的几何形状，也可以是复杂的图案和图形。图形通常是通过手绘、绘画、绘图软件或其他图形工具创建的，用于传达和表达特定的概念、思想或信息。图形的创作可以具有艺术性、装饰性或功能性的目的。

从广义的概念，图形一般分为几何属性和非几何属性：
...
使用图来表达信息，并不能完全消除文字表达的限制，如上下文负载、理解差异等。但是，相比于文字表达，图形表达具有以下优势：
