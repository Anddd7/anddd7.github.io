...
- 管理（CxO）：产品的领导者，关注愿景、方向、价值
- 产品（PM/PO）：产品设计者，通过功能设计实现价值
- 架构（Arch）：架构师，关注 CFR 和系统设计
- 研发（TL/Dev）：工程师，进行业务功能在系统上的实现

> 这里还有一个隐含的层次，因为组织的规模不同，同样的角色可能代表不同的职责范围。比如，世界 500 强的 CTO 和创业公司的 CTO 关注点是不同的，传统行业研发的架构师和互联网研发的架构师职责也不尽相同。为了避免深入到组织架构的讨论，这里的角色限于负责单一“软件产品”的团队内。
...
为了方便理清本文中架构图的信息边界，这里先对我们所讨论的软件产品下一个笼统的、仅供参考的定义。
...
当然，管理（CXO）和产品（产品经理、项目经理）关注的更多是非技术的信息，通常意义的架构图是不包含这些信息的。但在实际的开发过程中，为了保证开发流程的顺利进行，必要的业务知识是架构师和开发工程师也需要了解的。同样的，管理角色和产品经理能够了解基本的技术架构，也能够提升团队沟通的效率。

因此，我们也挑选出了一些常用的可视化工具作为非技术的“架构图”。接着，我们为每一种视图添加一个或多个已知的可视化工具，如 用户旅程图、C4、部署架构图等。最后，我们为视图添加可能的箭头和描述，以说明多个视图间的关系。

![Alt text](../../assets/README/image-23.png)

> 注：矩阵中所给的可视化工具（蓝字）仅作为参考，可根据实际情况选择熟悉的工具，只要能够表达当前角色所关注的信息主体即可。\
> 注：在成熟且稳定的团队中，甚至可以构建自己的一套风格来绘制相应的架构图，只要保证信息传递的完整性和一致性即可。

#### 风格（示例补充）

以一个智能物业系统为例... （随便想的一个例子，主要看图）
